from flask import Flask, render_template, request, jsonify, redirect, url_for, os
...
def detail(keyword):
    api_key = 'c827fe46-cb71-495c-9c2e-b5fe7c569b19'
    url = f'https://www.dictionaryapi.com/api/v3/references/collegiate/json/{keyword}?key={api_key}'
    response = requests.get(url)
    definitions = response.json()

    if not definitions:
        # Menampilkan halaman error jika kata tidak ditemukan
        return render_template("error.html", word=keyword, suggestions=None)
    if type(definitions[0]) is str:
        # Menampilkan halaman error dengan saran kata
